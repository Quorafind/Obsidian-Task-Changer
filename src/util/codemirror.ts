...
export const getSelection = (editor: Editor): Selection => {
  if (editor.somethingSelected()) {
    ...
    const start: EditorPosition = { line: cursorStart.line, ch: 0 };
...
export const prefixLines = ({
  ...
  if (preserveIndent) {
    ...
    lines.forEach((line, index) => {
      ...
      if (line.length == 0) {
        const blankcotent = line.replace(buildRegex([...replace, REGEX_ANY]), "");
        lines[index] = `${indent}${blankcotent}` ;
      } else {
